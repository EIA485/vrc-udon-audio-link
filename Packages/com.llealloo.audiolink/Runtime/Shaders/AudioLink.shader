        ...
        Pass
        {
            ...
            float4 frag (v2f_customrendertexture IN) : SV_Target
            {
                ...
                for(i = AUTOCORRELATOR_EBASEBIN; i < AUTOCORRELATOR_EMAXBIN; i++)
                {
                    ...
                    float phaseOffset = (bin.y - .5) * UNITY_PI;
                    float4 csv = float4(cos(frequency * wavePosition * fvr), cos(frequency * wavePosition * fvr + i * 0.32), cos(frequency * wavePosition * fvr + phaseOffset), cos(frequency * wavePosition * fvr + (i + phaseOffset) * 0.32));
                    csv.yw *= step(i % 4, 1) * 4.;
